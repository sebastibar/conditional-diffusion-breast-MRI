...
def generate_sample(model, test_dataset, noise_scheduler, accelerator, epoch, title="Generated Post"):
    """Helper function to generate and plot a sample during training."""
    model.eval()
    sample = test_dataset[0]
    pre_img = sample["pre_img"].unsqueeze(0)
    post_img = sample["post_img"].unsqueeze(0)
    
    # Move to device using accelerator
    ...
    plt.tight_layout(); 
    plt.savefig(f"{config.output_dir}/sample_epoch_{epoch+1}.png") # Save instead of show
    plt.close()
...
def main():
    ...
    model = ConditionalUNet(in_channels=2)
    ...
    for epoch in range(config.num_epochs):
        ...
        if avg_metrics["MAE"] < best_loss:
            ...
            # Save the unwrapped model from accelerator
        ...
        if accelerator.is_local_main_process: # Only generate on main process
            with ema.average_parameters():
